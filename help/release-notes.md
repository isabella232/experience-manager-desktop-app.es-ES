...
A continuación, encontrará información de la última versión de la aplicación de escritorio, 2.2.2. La fecha de la versión es el 9 de marzo de 2023.

La versión más reciente de la aplicación de escritorio incluye las siguientes correcciones de errores:
...
* [!DNL Experience Manager] 6.4.0 - 6.4.3 con [paquete de compatibilidad](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/featurepack/adobe-asset-link-support) instalado, en Adobe Managed Services (AMS) o en línea.
...
* Las rutas para colocar archivos en la aplicación nativa utilizan una ruta de escritorio global (desde el recurso compartido de red local que se muestra con) [!UICONTROL Reveal] opción).
...
* la aplicación de escritorio muestra el mensaje de error de tiempo de espera de sesión al hacer clic **[!UICONTROL Sign In]**.
...
* Una nueva opción para [!UICONTROL Clear Cookies] se añade al menú principal de la aplicación. Ayuda con posibles problemas de inicio de sesión, por ejemplo al cambiar la conexión de un servidor a otro. Consulte [borrar cookies antes de conectarse](/help/troubleshoot.md#cannot-login-cookies-issue).

* Se añade una opción que (si está seleccionada) permite a la aplicación cargar carpetas y archivos de modo que los nombres de sus nodos se creen en [!DNL Adobe Experience Manager] son los mismos que los nombres de archivo y carpeta locales.

   Este comportamiento es similar al comportamiento predeterminado en la versión 1 de la aplicación de escritorio. Mientras que en la versión actual, si la opción no está activada, se muestran los espacios en blanco y los caracteres `% ; # , + ? ^ { } "` en las carpetas, los nombres se sustituyen por guiones en las rutas de carpetas. Además, los caracteres en mayúsculas se convierten en minúsculas en las rutas de carpetas. Sin embargo, en los nombres de archivo, los caracteres `# % { } ? &` se sustituyen por guión, pero se conservan los espacios en blanco y la carcasa. Para obtener más información, consulte [Preferencias de aplicación](/help/install-upgrade.md#set-preferences) y [Carga y adición de nuevos recursos](/help/using.md#upload-and-add-new-assets-to-aem).

**Actualización en la aplicación 2.1.1.0**

* Una configuración avanzada permite que la aplicación emule el comportamiento de la aplicación v1.10 al cargar carpetas. En la versión 1.10, los nombres de nodo creados en el repositorio respetan los espacios y mayúsculas y minúsculas de los nombres de carpeta proporcionados por el usuario. El comportamiento predeterminado de v2.1 sigue siendo el mismo, es decir, reemplaza varios espacios en los nombres de carpeta con un guion en el nombre del nodo del repositorio y conviértelo en nombres de nodo en minúsculas. Consulte [las preferencias de aplicación](/help/install-upgrade.md#set-preferences).
...
* La configuración de aceleración de carga ahora está disponible para mejorar el rendimiento de carga. Cuando esta opción está activada, la aplicación se carga más rápido usando más subprocesos de CPU locales y consume más recursos.
...
* La aplicación de escritorio ahora enumera subcarpetas sin necesidad de actualizarlas explícitamente. <!-- CQ-4285711 -->
...
* La aplicación proporciona control total sobre las operaciones que requieren la transferencia completa de archivos binarios de recursos desde y hacia [!DNL Experience Manager] (abrir, editar, cargar cambios y cargar recursos).
...
* Dificultad al desplazarse por las carpetas o los resultados de búsqueda sin un panel de seguimiento o un puntero de ratón. Es posible que la barra de desplazamiento no aparezca con los dispositivos del mouse sin la rueda del mouse. <!-- CQ-4269947 -->
...
* En ocasiones, cuando se conecta a una dirección URL que no tiene [!DNL Experience Manager] servidor en ejecución, la pantalla de conexión deja de responder. Salga de la aplicación y vuelva a iniciarla.
