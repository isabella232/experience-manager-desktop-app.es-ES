---
cloud: experience-cloud
product: Aplicación de escritorio de Adobe Experience Manager
audience: end-user
user-guide-title: Ayuda de la aplicación de escritorio de AEM
solution-title: AEM Assets
solution-hub-url: https://helpx.adobe.com/es/experience-manager/6-5/assets/user-guide.html
getting-started-url: https://docs.adobe.com/content/help/es-ES/experience-manager-desktop-app/using/introduction.translate.html
tutorials-title: Tutoriales de AEM
tutorials-url: https://helpx.adobe.com/experience-manager/kt/index/aem-tutorials.html
translation-type: ht
source-git-commit: ffc67d223f4db3a3a91cba5dfb5f725153319b02

---


# Guía del usuario de la aplicación de escritorio de Adobe {#using}

+ [Información general](introduction.md)
+ [Notas de la versión](release-notes.md)
+ [Instalación o actualización](install-upgrade.md)
+ [Uso de la aplicación](using.md)
+ [Solución de problemas](troubleshoot.md)
+ Ayuda de la versión anterior {#archive}
   + [Notas de la versión v1](release-notes-of-v1.md)
   + [Instalar y configurar v1](install-configure-app-v1.md)
   + [Uso de la aplicación v1](use-app-v1.md)
   + [Prácticas recomendadas v1](best-practices-for-v1.md)
   + [Solución de problemas v1](troubleshoot-app-v1.md)
