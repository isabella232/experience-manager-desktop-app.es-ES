...
* Se ha añadido compatibilidad con el inicio de sesión IMS. La integración de IMS permite a la aplicación de escritorio actualizar el token de acceso automáticamente, lo que permite al usuario permanecer conectado durante un máximo de 14 días.
...
* Una nueva opción para [!UICONTROL Clear Cookies] se añade al menú principal de la aplicación. Ayuda con posibles problemas de inicio de sesión, por ejemplo al cambiar la conexión de un servidor a otro. Consulte [borrar cookies antes de conectarse](/help/using/troubleshoot.md#cannot-login-cookies-issue).
...
* Se añade una opción que (si está seleccionada) permite a la aplicación cargar carpetas y archivos de modo que los nombres de sus nodos se creen en [!DNL Adobe Experience Manager] son los mismos que los nombres de archivo y carpeta locales.
  ...
  Este comportamiento es similar al comportamiento predeterminado en la versión 1 de la aplicación de escritorio. Mientras que en la versión actual, si la opción no está activada, se muestran los espacios en blanco y los caracteres `% ; # , + ? ^ { } "` en las carpetas, los nombres se sustituyen por guiones en las rutas de carpetas. Además, los caracteres en mayúsculas se convierten en minúsculas en las rutas de carpetas. Sin embargo, en los nombres de archivo, los caracteres `# % { } ? &` se sustituyen por guión, pero se conservan los espacios en blanco y la carcasa. Para obtener más información, consulte [Preferencias de aplicación](/help/using/install-upgrade.md#set-preferences) y [Carga y adición de nuevos recursos](/help/using/using.md#upload-and-add-new-assets-to-aem).

**Actualización en la aplicación 2.1.1.0**

* Una configuración avanzada permite que la aplicación emule el comportamiento de la aplicación v1.10 al cargar carpetas. En la versión 1.10, los nombres de nodo creados en el repositorio respetan los espacios y mayúsculas y minúsculas de los nombres de carpeta proporcionados por el usuario. El comportamiento predeterminado de v2.1 sigue siendo el mismo, es decir, reemplaza varios espacios en los nombres de carpeta con un guion en el nombre del nodo del repositorio y conviértelo en nombres de nodo en minúsculas. Consulte [las preferencias de aplicación](/help/using/install-upgrade.md#set-preferences).
